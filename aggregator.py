...
class ModelLoader:
    ...
    def _load_all(self):
        ...
        for disease, config in MODEL_REGISTRY.items():
            ...
            print("Looking for model at:", path)   # DEBUG
...
def validate_and_fill(self, patient_data: dict, disease: str) -> pd.DataFrame:
    ...
    # Build row: use patient value if present, else default
        row = {}
...
def predict_single(model, patient_data: dict, disease: str) -> dict:
    ...
    try:
        X = ModelLoader.validate_and_fill(patient_data, disease)
...
class ClinicalDecisionAggregator:
    ...
    def __init__(self, threshold: float = 0.30):
        """
        threshold: minimum probability to include in output.
                   Diseases below this are 'Low risk' and filtered.
    ...
    def predict(self, patient_data: dict, top_n: int = 3) -> dict:
        ...
        # Split into flagged (above threshold) and low-risk
